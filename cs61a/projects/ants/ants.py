...
class Ant(Insect):
    ...
    def can_contain(self, other_ant):
        if self.container and not self.ant and not other_ant.container:
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        index = 0
        while True:
            if location == hive or location.entrance == None:
                return None
            elif location.bees != [] and self.min_range <= index <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor == 0:
            bees_alive = list(i for i in curr_place.bees)
            for bee in bees_alive:
...
"*** REPLACE THIS LINE ***"
class WallAnt(Ant):
    """A Wall Ant that can block the path"""
    name = 'Wall'
    implemented = True
    food_cost = 4

    def __init__(self, armor=4):
        """Create a Wall Ant with an ARMOR quantity."""
        Insect.__init__(self, armor)
...
class ScubaThrower(ThrowerAnt):
    """A ScubaThrower that is able to be placed on water safely."""
    name = 'Scuba'
    food_cost = 6
    watersafe = True
    implemented = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def buff_ants(self):
        ...
        while True:
            if location.exit == None:
                return
            elif location.ant != None and location.ant.damage_buffed == False:
                location.ant.damage = location.ant.damage * 2
                location.ant.damage_buffed = True
            if hasattr(location.ant, 'container') and location.ant.container:
                if location.ant.ant != None and location.ant.ant.damage_buffed == False:
                    location.ant.ant.damage = location.ant.ant.damage * 2
                    location.ant.ant.damage_buffed = True
...
def apply_effect(effect, bee, duration):
    ...
    def effect_action(colony):
        nonlocal duration_count
        nonlocal default_action
        duration_count += 1
        if duration_count <= duration:
            return effect(default_action)(colony)
        return default_action(colony)
