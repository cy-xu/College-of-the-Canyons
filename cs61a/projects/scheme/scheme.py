...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        all_operands = rest.map(lambda operand: scheme_eval(operand, env))
        check_procedure(operator)
        return scheme_apply(operator, second.map(lambda x: scheme_eval(x,env,True)), env)
...
def do_and_form(expressions, env):
    ...
    the_expressions = expressions
    if isinstance(the_expressions.first, Pair):
        if scheme_truep(scheme_eval(the_expressions.first, env)):
            the_expressions = the_expressions.second
            return do_and_form(the_expressions, env)
        else:
            return False
    if scheme_truep(scheme_eval(the_expressions.first, env)):
        the_expressions = the_expressions.second
...
def do_or_form(expressions, env):
    ...
    if isinstance(the_expressions.first, Pair):
        ...
        if not scheme_truep(scheme_eval(the_expressions.first, env)):
            the_expressions = the_expressions.second
            return do_or_form(the_expressions, env)
        else:
            return scheme_eval(the_expressions.first, env, True)
    if not scheme_truep(scheme_eval(the_expressions.first, env)):
        the_expressions = the_expressions.second
        return do_or_form(the_expressions, env)
    else:
