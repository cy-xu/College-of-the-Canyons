...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    assert type(num_rolls) == int, 'num_rolls must be an integer.'
    assert num_rolls >= 0, 'Cann1ot roll a negative number of dice in take_turn.'
    ...
    score = hog_prime(score)
    if score > 25 - num_rolls:#When Pigs Fly
        score = 25 - num_rolls
    return score
...
def swine_swap(score, opponent_score):#added after
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while(score0 < goal and score1 < goal):
        if player == 0:
            ...
            if player0_num_rolls == -1:#pork chop
                score0 += 1
                dice_swapped = swap_dice(dice_swapped)
            else:
                score0 += take_turn(player0_num_rolls, score1, select_dice(score0,score1,dice_swapped))
            score0,score1 = swine_swap(score0,score1)
            player = 1
        elif player == 1:
            player1_num_rolls = strategy1(score1,score0)
            if player1_num_rolls == -1:
                score1 += 1
                dice_swapped = swap_dice(dice_swapped)
            else:
                score1 += take_turn(player1_num_rolls, score0, select_dice(score1,score0,dice_swapped))
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    if hog_prime(free_bacon(opponent_score)) >= margin or hog_prime(free_bacon(opponent_score)) + score == opponent_score*2:
        return 0
    return num_rolls
    # END PROBLEM 10
...
def final_strategy(score, opponent_score):
    ...
    *** Start the game by using Pork Chop with a roll of -1.  
        Use Free Bacon whenever the player's score is above 90 or whenever
        the player is ahead by at least 10 points. ***
    """
    # BEGIN PROBLEM 11
    if score == 0:
        return -1
