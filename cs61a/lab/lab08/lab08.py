...
def make_to_string(front, mid, back, empty_repr):
    ...
    def str_link(lnk):
        if lnk == Link.empty:
            return empty_repr
        return front + str(lnk.first) + mid + str_link(lnk.rest) + back
    return str_link
...
def tree_map(fn, t):
    ...
    t.branches = [tree_map(fn,b) for b in t.branches]
    return Tree(fn(t.root),t.branches)
...
def add_trees(t1, t2):
    ...
    "*** YOUR CODE HERE ***"
    if not t1:
        return Tree(t2.root,t2.branches)
    if not t2:
        return Tree(t1.root,t1.branches)
    if len(t1.branches) < len(t2.branches):
        t1.branches = t1.branches + [Tree(0) for i in range(len(t2.branches)-len(t1.branches))]
    elif len(t1.branches) > len(t2.branches):
        t2.branches = t2.branches + [Tree(0) for i in range(len(t1.branches)-len(t2.branches))]
    return Tree(t1.root + t2.root,[add_trees(x,y) for x,y in zip(t1.branches,t2.branches)])
